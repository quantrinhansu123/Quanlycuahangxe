-- Migration: 202609110004_purchase_receipt_security_and_baseline_hardening.sql
-- Description: Security hardening (search_path) cho RPC functions va enforce database lam nguon su that sinh ma phieu nhap hang

-- 1. Hardening search_path cho function lay ma phieu
...
-- 2. Hardening RPC save_purchase_receipt:
--    - Dat search_path = public, pg_temp
--    - Database la nguon quyet dinh ma phieu duy nhat khi tao moi (luon lay nextval, khong tin ma_phieu tu client)
--    - Khi update: giu nguyen ma_phieu goc tu database
--    - Enforce validation so_luong > 0, gia_nhap >= 0
--    - Bao ve tuyet doi ton_dau_ky trong ds_san_pham (khong bao gio ghi de ton_dau_ky)
...
BEGIN
  ...
  -- Validate tung item va tinh tong tien (khong tu dong sua data)
...
-- 3. Hardening RPC delete_purchase_receipt
...
BEGIN
  ...
  -- Xoa phieu master (cascade se tu xoa chi tiet)
