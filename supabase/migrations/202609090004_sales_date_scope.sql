-- Filter base sales by date before detail aggregation; global first-visit dates remain intact.
BEGIN;
CREATE INDEX IF NOT EXISTS sales_date_page ON public.the_ban_hang(ngay DESC, gio DESC, id DESC);
-- Classification needs only a customer identity and first date, not images,
-- recovered names, order details, services or complete order JSON.
...
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$
 WITH customers AS MATERIALIZED (
   ...
   SELECT DISTINCT ON (ref) ref, id FROM (
     SELECT id ref, id, 0 priority FROM customers
     UNION ALL SELECT code, id, 1 FROM customers
 ...
 ) SELECT key, min(ngay)::text FROM identities GROUP BY key;
$$;
CREATE OR REPLACE FUNCTION public.app_sales_rows_scoped(p_start date, p_end date) RETURNS SETOF jsonb
LANGUAGE sql STABLE SECURITY INVOKER SET search_path = public AS $$
 WITH customers AS MATERIALIZED (
...
CREATE OR REPLACE FUNCTION public.sales_query(
 ...
 SELECT j FROM app_sales_rows_scoped(p_start, p_end) j WHERE
