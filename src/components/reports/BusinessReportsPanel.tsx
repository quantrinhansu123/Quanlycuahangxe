  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-[11px] text-muted-foreground">Kỳ so sánh: {dateVi(data.previousStart)} – {dateVi(data.previousEnd)} (cùng số ngày với kỳ đang chọn).</p>
      </div>
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between gap-3"><h3 className="font-black text-foreground">Công nợ phải thu / phải trả</h3><span className="text-xs text-muted-foreground">Tổng còn nợ: {money(data.debts.reduce((sum, row) => sum + row.con_no, 0))}</span></div>
      ...
      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <section className="space-y-2">
          <div className="flex items-center justify-between"><h3 className="font-black text-foreground">Chi phí theo danh mục</h3><span className="text-xs font-bold text-rose-600">{money(data.totalExpenses)}</span></div>
        ...
        <section className="space-y-2">
          <div className="flex items-center justify-between"><h3 className="font-black text-foreground">Dòng tiền theo phương thức</h3><span className="text-xs text-muted-foreground">Thu {money(data.totalCashIn)} · Chi {money(data.totalCashOut)}</span></div>
      ...
      <section className="space-y-2">
        ...
        <div className="overflow-x-auto rounded-xl border border-border"><table className="w-full text-xs"><thead className="bg-muted/50 text-muted-foreground"><tr><th className="px-4 py-3 text-left">Mã</th><th className="px-4 py-3 text-left">Sản phẩm</th><th className="px-4 py-3 text-right">Đầu kỳ</th><th className="px-4 py-3 text-right">Nhập</th><th className="px-4 py-3 text-right">Xuất</th><th className="px-4 py-3 text-right">Cuối kỳ</th></tr></thead><tbody className="divide-y divide-border">{data.inventory.length === 0 ? <EmptyRow cols={6} /> : data.inventory.map((row) => <tr key={row.id}><td className="px-4 py-3">{row.ma_hang || '—'}</td><td className="px-4 py-3 font-semibold text-foreground">{row.ten_hang}</td><td className="px-4 py-3 text-right">{number(row.dau_ky_so_luong)}</td><td className="px-4 py-3 text-right text-emerald-600">{number(row.nhap_so_luong)}</td><td className="px-4 py-3 text-right text-rose-600">{number(row.xuat_so_luong)}</td><td className="px-4 py-3 text-right font-bold">{number(row.cuoi_ky_so_luong)}</td></tr>)}</tbody></table></div>
      </section>
    </div>
  );
}
