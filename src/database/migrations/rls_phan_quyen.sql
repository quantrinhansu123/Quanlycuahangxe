...
-- Hàm kiểm tra admin: khớp AuthContext (vi_tri chứa "Quản trị viên" / "admin")
-- + Chủ cửa hàng / quản lý. Trước đây IN tuyệt đối từ chối chuỗi có thêm mô tả.
...
-- RLS bảng cham_cong: EXISTS (subquery từ nhan_su) bị lọc bởi RLS nhan_su → dùng hàm DEFINER
...
-- Tên cũ từng gộp OR (khi re-run script)
...
-- Một lệnh = một policy: (admin ghi/xem/xóa hết) OR (nv chỉ bản thân, qua hàm DEFINER)
-- Tránh tách "FOR ALL admin" + policy nv (một số bản PG/PostgREST gây INSERT fail dù is_admin = true)
