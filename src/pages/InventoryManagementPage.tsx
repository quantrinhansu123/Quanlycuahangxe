...
import { deleteInventoryRecord, bulkUpsertInventoryRecords, deleteAllInventoryRecords, getInventoryPaginated, getInventoryRecords, getProductRecords, upsertProductsFromInventory } from '../data/inventoryData';
import type { InventoryRecord, ProductRecord } from '../data/inventoryData';
...
const InventoryManagementPage: React.FC = () => {
  ...
  const [visibleColumns, setVisibleColumns] = useState<string[]>([
    'id_xuat_nhap_kho', 'loai_phieu', 'id_don_hang', 'co_so', 'ten_mat_hang', 'ton_dau_ky', 'so_luong', 
    ...
    <div className="w-full flex-1 animate-in fade-in slide-in-from-bottom-4 duration-500 text-muted-foreground font-sans">
      ...
      <div className="mt-4 bg-card rounded-lg border border-border shadow-sm overflow-hidden">
        <div className="px-4 py-3 border-b border-border bg-muted/40">
          <h3 className="text-[14px] font-bold text-foreground">DS sản phẩm</h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-muted border-b border-border text-muted-foreground text-[12px] font-bold uppercase tracking-wider">
                <th className="px-4 py-3 font-semibold">Mã sản phẩm</th>
            ...
            <tbody className="divide-y divide-slate-100 text-[13px]">
              {products.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-muted-foreground">Chưa có dữ liệu sản phẩm.</td>
                </tr>
              ) : products.map((p) => (
                <tr key={p.id} className="hover:bg-muted/80 transition-colors">
                  <td className="px-4 py-3 text-muted-foreground">{p.ma_san_pham || '—'}</td>
                  <td className="px-4 py-3 font-semibold text-foreground">{p.ten_san_pham}</td>
                  <td className="px-4 py-3 text-muted-foreground">{p.don_vi_tinh || '—'}</td>
                  <td className="px-4 py-3 text-right font-bold text-foreground">{formatNumber(p.ton_dau_ky)}</td>
                </tr>
              ))}
            </tbody>
          </table>
